...
var avatarTemplateText = 'SCREEN:\n\t(set: $Screen to "Screen{NAME}")\n\nSTAGE:\n\tScene:\n\t\t(set: $Model to "SceneModel{NAME}")\n\t\t(set: $Anim to "SceneAnim{NAME}")\n\t\t(set: $Lighting to "SceneLighting{NAME}")\n\t\t(set: $Sound to "Sound{NAME}")\n\t\t(set: $Effect to "SoundEffect{NAME}")\n\t\t(set: $Duration to "{NAME}")\n\nOBJECTS(2):\n\tObject1:\n\t\t(set: $Model to "ObjectModel{NAME}") \n\t\t(set: $Origin to "({X, Y, Z})")\n\t\t(set: $Anim to "ObjectAnim{NAME}")\n\t\t(set: $Destination to "({X, Y, Z})")\n\t\t(set: $Duration to "{NAME}")\n\t\t(set: $Lighting to "ObjectLighting{NAME}")\n\t\t(set: $Sound to "ObjectSound{NAME}")  \n\t\t(set: $Duration to "{NAME}")  \n\tObject2:\n\t\t(set: $Model to "ObjectModel{NAME}")\n\t\t(set: $Origin to "({X, Y, Z})")\n\t\t(set: $Anim to "ObjectAnim{NAME}")\n\t\t(set: $Destination to "({X, Y, Z})")\n\t\t(set: $Duration to "{NAME}")\n\t\t(set: $Lighting to "ObjectLighting{NAME}")\n\t\t(set: $Sound to "ObjectSound{NAME}")  \n\t\t(set: $Duration to "{NAME}")  \n\nAVATARS:\n\n[[{NextNodeTitle}]]';
	...
	mutations: {
		...
		CREATE_PASSAGEA_IN_STORY(state, storyId, props) {
			...
			*/
			...
			var avatarBlock = '\n\tAvatar:\n\t\tVoice:\n\t\t\t"{INSERT DIALOGUE HERE}"\n\t\t\t(set: $Name to "VoiceName{NAME}")\n\t\tSound\n\t\t\t(set: $Effect to "SoundEffect{NAME}")\n\t\tFace:\n\t\t\t(set: $Model to "FaceModel{NAME}")\n\t\t\t(set: $Anim to "FaceAnim{NAME}")\n\t\t\t(set: $Axis to "FaceAxis{NAME}")\n\t\t\t(set: $Duration to "{NAME}")\n\t\tBody:\n\t\t\t(set: $Model to "BodyModel{NAME}")\n\t\t\t(set: $Origin to "({X, Y, Z})")\n\t\t\t(set: $Anim to "BodyAnim{NAME}")\n\t\t\t(set: $Destination to "({X, Y, Z})") \n\t\t\t(set: $Duration to "{NAME}")\n\n';
			var objectBlock = '\n\tObject:\n\t\t(set: $Model to "ObjectModel{NAME}") \n\t\t(set: $Origin to "({X, Y, Z})")\n\t\t(set: $Anim to "ObjectAnim{NAME}")\n\t\t(set: $Destination to "({X, Y, Z})")\n\t\t(set: $Duration to "{NAME}")\n\t\t(set: $Lighting to "ObjectLighting{NAME}")\n\t\t(set: $Sound to "ObjectSound{NAME}")  \n\t\t(set: $Duration to "{NAME}")';
