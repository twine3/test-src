...
const { createPassage,createPassageA,createPassageX,createPassageZ, createPassageL,createPassageU,  deletePassage, positionPassage, updatePassage } = require('../data/actions/passage');
...
module.exports = Vue.extend({
	...
	ready() {
		...
		if (this.story.passages.length === 0) {
			this.createPassageAt("Synopsis",0,0);
			this.story.passages.forEach(p => {
				if (p.name == "Synopsis") {
					p.text = "\nCHARACTERS:\n\t[AvatarA, AvatarB]\n\nVOICES:\n\t\nSCENE:\n\t\nSCENARIO:\n\t";
				}
		...
		*/
		...
		createPassageAtS(name, top, left) {
			...
			*/
			...
			this.createPassageS(this.story.id, { name, left, top });
